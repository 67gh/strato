        ...
        if (!texture || texture->texture->replaced) {
            ...
            switch (textureHeader.textureType) {
                ...
                case TextureImageControl::TextureType::e1DBuffer:
                    // 1D Buffer textures used by NVN for particle/skinning data.
                    // Map to 1D image view — closest Vulkan equivalent.
                    // Avoids crashing when games bind buffer textures.
            ...
            if (textureHeader.headerType == TextureImageControl::HeaderType::Pitch) {
                ...
                // Occurs with NVN header types introduced after fw 12.x.
