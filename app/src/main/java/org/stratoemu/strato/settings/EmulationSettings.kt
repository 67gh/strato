...
class EmulationSettings private constructor(context : Context, prefName : String?) {
    ...
    /**
     * Render resolution multiplier, stored as a String because it's backed by a ListPreference.
     * Valid values: "0.5", "0.75", "1.0", "1.5", "2.0"
     */
    var renderScaleFactor by sharedPreferences(context, "1.0", prefName = prefName)
    /**
     * Performance mode, stored as a String (ListPreference), parsed to Int by AndroidSettings.
     * "0" = Normal, "1" = Extreme Compatibility, "2" = Extreme Performance
     */
    var performanceMode by sharedPreferences(context, "0", prefName = prefName)
